...
myApp.services = {
  ...
  tasks: {
    ...
    create: function (data) {
      // Task item template.
      //let ischeck = taskItem.data.completed;
      ...
      if (data.completed) {
        var taskItem = ons.createElement(
            ...
            '<label class="left">' +
            '<ons-checkbox checked ="true" ></ons-checkbox>' +
            '</label>' +
            '<div class="center">' +
            data.title +
            '</div>' +
      ...
      if (!data.completed) pendingList = document.querySelector('#pending-list');
      else pendingList = document.querySelector('#completed-list');
      pendingList.insertBefore(taskItem, taskItem.data.urgent ? pendingList.firstChild : null);

      myApp.services.tasks.update(taskItem, taskItem.data);
    },
    ...
    update: function(taskItem, data) {
      ...
      if (data.encours && !data.completed) pendingList = document.querySelector('#encours-list');
      else {
        if (!data.completed) pendingList = document.querySelector('#pending-list');
        else pendingList = document.querySelector('#completed-list');
      }
    ...
    // Deletes all tasks item and its listeners.
    removeAll() {
      localStorage.clear();
  ...
  animators: {
    ...
    // Animation de swipe quand la tache est completee
    ...
    // Animation de swipe quand la tache est suppimee
